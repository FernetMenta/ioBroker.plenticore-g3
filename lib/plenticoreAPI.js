...
class PlenticoreAPI {
     ...
     */
    constructor(host, port, useHTTPS, password, timeout, logger) {
        this.loggedIn = false;
        this.#logger = logger;
        this.#host = host;
        this.#port = port;
    ...
    async apiCall(method, endpoint, data) {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.on('response', response => {
                ...
                response.on('end', () => {
                    ...
                    if (code >= 200 && code <= 299) {
                        ...
                        this.#logger.debug(`Result of API request: code: ${code}, headers: ${headers}, body: ${body}`);
                    } else {
                        if (this.#errorCounter <= 5) {
                            this.#logger.warn(`API request failed: code: ${code}, headers: ${headers}, body: ${body}`);
                        }
                        this.#errorCounter++;
                    }

                    if (this.#errorCounter == 6) {
                        this.#logger.error('too many api call errors, error logging suspended');
                    }

                    if (this.#errorCounter > 5) {
                        this.loggedIn = false;
                        reject(new Error('too many api call errors'));
                    }

                    resolve({ body: json, code: code, headers: headers });
                });
            });
            ...
            request.on('error', error => {
                this.loggedIn = false;
                if (this.#errorCounter <= 5) {
                    this.#logger.error(`API request failed with error ${JSON.stringify(error)}`);
                }
                this.#errorCounter++;
