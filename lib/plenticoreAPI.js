...
class PlenticoreAPI {
  ...
  async apiCall (method, endpoint, data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.#logger.debug(
        'Making request to endpoint ' + endpoint + ' with data ' + JSON.stringify(reqOpts)
      );
      request.on('response', response => {
        ...
        response.on('end', () => {
          ...
          {
            this.loggedIn = false;
            throw new Error('to many api call errors');
          }

          return resolve({ body: body, code: code, headers: headers });
        });
      });
  ...
  async login () {
    ...
    try {
      ...
      if (ret.code == 401 || ret.code == 403) {
        this.#logger.warn('Login failed with code ' + ret.code + ': ' + ret.body);
    ...
    if (!json.nonce) {
      this.#logger.warn('No nonce in json reply to start: ' + body);
      return Promise.resolve(-3);
    }
    ...
    var json = JSON.parse(ret.body);
    if (!json.sessionId) {
      this.#logger.warn('No session id in json reply to create session: ' + body);
      return Promise.resolve(-5);
    }
