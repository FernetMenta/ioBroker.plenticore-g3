...
const presetProcessDataIDs = [
    { id: 'devices:local/Inverter:State', description: 'Inverter:State', role: 'value', required: true },
    { id: 'devices:local/Grid_P', description: 'Grid_P', role: 'value.power', required: true },
    { id: 'devices:local/Dc_P', description: 'Dc_P', role: 'value.power', required: true },
    { id: 'devices:local/Home_P', description: 'Home_P', role: 'value.power', required: true },
    { id: 'devices:local:ac/P', description: 'ac/P', role: 'value.power', required: false },
    { id: 'devices:local:battery/P', description: 'battery/P', role: 'value.power', required: false },
    { id: 'devices:local:battery/SoC', description: 'battery/SoC', role: 'level.battery', required: false },
    {
        id: 'scb:statistic:EnergyFlow/Statistic:Yield:Day',
        description: 'Statistic:Yield:Day',
        role: 'value.energy',
        required: true,
    },
    {
    ...
    { id: 'scb:update/Update:Version', description: 'Update:Version', role: 'value', required: true },
];
...
const presetSettingIDs = [
    {
        id: 'devices:local/Battery:MinHomeComsumption',
        description: 'Battery:MinHomeComsumption',
        role: 'level',
        required: false,
    },
    {
        id: 'devices:local/Battery:MinSoc',
        description: 'Battery:MinSoc',
        role: 'level.battery',
        required: false,
    },
    { id: 'scb:update/Update:Method', description: 'Update:Method', role: 'state', required: true },
];
...
const PollInverterState = [
    {
        moduleid: 'devices:local',
        processdataids: ['Inverter:State'],
    },
];
...
class PlenticoreData {
    ...
    #createPollIDs() {
        this.#pollIDs = [];
        let hasBattery = this.#adapter.getBatteryPresence();
        if (this.#type == 'processdata') {
            hasBattery = this.#allIDs.find(element => element.id.startsWith('devices:local:battery'));
            this.#adapter.setBatteryPresence(hasBattery);
        }
        for (const preset of this.#presetIDs) {
            if (
                (preset.id.startsWith('devices:local:battery') || preset.id.startsWith('devices:local/Battery')) &&
                !hasBattery
            ) {
                continue;
            }
            ...
            if (foundInAllIDs) {
                ...
                if (preset.required) {
                    this.#adapter.log.warn(`preset not found in list: ${preset.id}`);
                } else {
                    this.#adapter.log.debug(`preset not found in list: ${preset.id}`);
                }
            }
        }
        for (const optional of this.#optionalIDs) {
            ...
            if (foundInAllIDs && foundInAllIDs.preselected) {
                continue;
            }
            if (foundInAllIDs) {
                ...
                this.#adapter.log.warn(`optional not found in list: ${optional.id}`);
            }
        }
    }
    ...
    async #createObjects(values) {
        for (const module of values) {
            ...
            for (const value of module[this.#type]) {
                ...
                if (found) {
                    description = this.#I18n.getTranslatedObject(found.description);
                }
                ...
                if (this.#type == 'processdata') {
                    ...
                    if (!pdata) {
                        this.#adapter.log.warn(`processdata not found in allIDS: ${module.moduleid}/${value.id}`);
                        continue;
                    }
                    let role =
                    ...
                    if (!setting) {
                        this.#adapter.log.warn(`setting not found in allIDS: ${module.moduleid}/${value.id}`);
                        continue;
                    }
    ...
    /**
     * Gets poll ID of inverter state
     */
    static getPollInverterStateID() {
        return PollInverterState;
    }
    ...
    async init(optionalIDs, allAdapterObjs) {
        ...
        for (const preset of this.#presetIDs) {
            ...
            if (found) {
                found.description = this.#I18n.getTranslatedObject(preset.description);
        ...
        this.#createPollIDs();
        // only delete if inverter has already reached state FeedIn
        if (allAdapterObjs !== undefined) {
