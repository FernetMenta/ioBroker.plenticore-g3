...
class PlenticoreData {
    ...
    async #deleteUnusedObjects(allObjects) {
        for (const [key, value] of Object.entries(allObjects)) {
            if (value.type == 'state') {
                ...
                let topChannel = match[1];
                let moduleId = match[2].replaceAll('.', ':');
                let id = match[3];
        ...
        for (const module of values) {
            ...
            for (const value of module[this.#type]) {
                ...
                if (this.#type == 'processdata') {
                    await this.#adapter.setObject(`${channelname}.${value.id}`, {
                        type: 'state',
                        common: {
                            name: value.id.replace('Statistic:', ''),
                            desc: description,
                            type: 'number',
                            role: 'value',
                            unit: value.unit,
    ...
    async processData(values) {
        ...
        for (const module of values) {
            ...
            for (const value of module[this.#type]) {
                if (this.#type == 'processdata') {
                    this.#adapter.setState(`${channelname}.${value.id}`, value.value, true);
                } else {
                    let obj = await this.#adapter.getObjectAsync(`${channelname}.${value.id}`);
                    let type = obj.common.type;
                    let val = type == 'string' ? value.value : Number(value.value);
                    this.#adapter.setState(`${channelname}.${value.id}`, val, true);
                }
            }
        }
    }
     ...
     *
     * @param {string} id - id of the iobroker state
     * @param {state} state - iobroker state
     */
    static prepareWriteValue(id, state) {
        let tmp = id.substring(id.indexOf('.') + 1);
        tmp = tmp.substring(tmp.indexOf('.') + 1);
        tmp = tmp.substring(tmp.indexOf('.') + 1);
        let moduleid = tmp.substring(0, tmp.lastIndexOf('.')).replaceAll('.', ':');
        let settingid = tmp.substring(tmp.lastIndexOf('.') + 1).replaceAll('.', ':');
        let payload = [
            {
                moduleid: moduleid,
                settings: [
